...
> Status: **Phases 1–3 complete**; **Phase 4 harness + CI built and tested**
> (live eval numbers pending model quota). The correct-or-silent agent is in
> (plan contract, schema linking, template-first SQL, self-consistency,
> abstention, programmatic number injection). See [docs/agent.md](docs/agent.md),
...
<!-- EVAL:START -->
_Pending a live run._ The harness, metrics (EX, confident-wrong rate, abstention
recall, over-abstention), and risk-coverage sweep are built and tested; populate
this section with one command once model quota is available:

```sh
peerlens eval --update-readme    # writes the metrics table, the chosen τ, and the risk-coverage plot
```

Method: [docs/evaluation.md](docs/evaluation.md). The 20-question gold set lives
in `src/peerlens/eval/gold.json`.
<!-- EVAL:END -->
...
- **Phase 4** ◐ — Evaluation harness, metrics, risk-coverage sweep + operating-point
  picker, and CI **built and tested** ([docs/evaluation.md](docs/evaluation.md)).
  Live numbers pending model quota (`peerlens eval`). Remaining: Scorecard/Pell
  augmentation, HF Spaces demo, architecture diagram, MARKETview-stack mapping.
